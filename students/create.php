<?php 
	...
	if (isset($_POST['submit'])) {
		...
		if (count($errors) == 0) {
			$stmt = $conn->prepare("INSERT INTO `students`(`first_name`, `last_name`, `course_id`, `course_listened`) VALUES(:first_name, :last_name, :course_id, :course_listened)");
			$stmt = $stmt->execute([
			...
			<div class="form-group">
				...
				<select name="course_id">
					...
					<?php foreach ($courses as $key => $value) { ?>
						<option value="<?php echo $value['id'] ?>"  <?php echo $value['id'] == ($input_data['course_id']??false) ? 'selected' : '' ?>> <?php echo $value['course_name'] ?></option>
					<?php } ?>
