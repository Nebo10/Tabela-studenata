<?php 
	...
	if (isset($_POST['submit'])) {
		...
		if (count($errors) == 0) {
			$stmt = $conn->prepare("INSERT INTO `instructors`(`first_name`, `last_name`, `technology`) VALUES(:first_name, :last_name, :technology)");
			$stmt = $stmt->execute([
			...
			unset($input_data);
			$flash_msg = '<p class="text-success">Uspješno ste dodali instruktora.</p>';
		}
	}
?>
	...
	<div class="container">
		...
		<form action="#" method="POST">
			<div class="form-group">
				...
				<input type="text" class="form-control" value="<?php echo $input_data['first_name']??'' ?>" id="first_name" placeholder="Ime" name="first_name">
				<?php echo $errors['first_name']??'' ?>
			</div>
			<div class="form-group">
				<label for="last_name">Prezime</label>
				<input type="text" class="form-control" value="<?php echo $input_data['last_name']??'' ?>" id="last_name" placeholder="Prezime" name="last_name">
				<?php echo $errors['last_name']??'' ?>
			</div>

			<div class="form-group">
				<label for="technology">Programski jezici</label>
				<input type="text" class="form-control" value="<?php echo $input_data['technology']??'' ?>" id="technology" placeholder="Programski jezici" name="technology">
